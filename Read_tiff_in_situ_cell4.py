...
'''
insitu_file = 'img_04_aligned.tif'
...
'''
frame_number = np.array([0, 15, 16, 35, 36, 50, 57])
...
plt.imshow(log_insitu[showframe], interpolation= None, cmap = "autumn", vmin=2.4, vmax=4.4) #interpolation can be 'nearest' also.
...
#y_0 = 1290
#y_1 = y_0 + 30
#plt.plot( [x_0, x_1], [y_0, y_0], 'g-')
#plt.plot( [x_0, x_1], [y_1, y_1], 'g-')
#plt.plot( [x_0, x_0], [y_0, y_1], 'g-')
#plt.plot( [x_1, x_1], [y_0, y_1], 'g-')
#
##ROI_ii
#x_0 =  1600
#x_1 =  1880
...
plt.savefig('ROI_cell4_FOV.png', dpi = 300)
...
color_bar = plt.contourf(Z, np.rint(levels), cmap='autumn') #plt.cm.autumn)
plt.close()
...
for k , l in zip (range(0, insitu.shape[0]), color_idx):    
    plt.plot(width_x, pixel_y [k,:], label = k, color=plt.cm.autumn(l))
...
ax = plt.gca()
y_max = np.around(np.amax(pixel_y)+0.02, decimals = 2)
y_min = np.around(np.amin(pixel_y)-0.015, decimals = 2)
ax.tick_params(direction='in', colors='g', length=4, width=2)
plt.yticks(np.arange(y_min, y_max, 0.07), fontsize=12, color='g', fontweight='bold')
x_max = np.around(np.ceil(np.amax(width_x)), decimals = 2)
x_min = x_1 - x_1
plt.xticks(np.arange(x_min, x_max, 1.5), fontsize=12, color='g', fontweight='bold')
...
cbar.ax.tick_params(labelsize=12)
imag_name = 'ROI_iv_01' + '(x_0 = ' + str(x_0) + ', x_1 = ' + str(x_1) + ', y_0 = ' + str(y_0) + ', y_1 = ' + str(y_1) + ').png'
